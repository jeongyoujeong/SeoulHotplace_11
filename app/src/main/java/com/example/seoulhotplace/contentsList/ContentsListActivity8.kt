...
class ContentsListActivity8 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_contents_list8)

        val items = ArrayList<ContentModel>()
        val itemKeyList = ArrayList<String>()

        val button8 = intent.getStringExtra("button8")
        rvAdapter = ContentRVAdapter(baseContext, items, itemKeyList, bookmarkIdList)


        // Write a message to the database
        val database = Firebase.database
        val category = intent.getStringExtra("button8")


        myRef = database.getReference("button8")
